...
class Service(Cog_Extension):
        ...
        try:
            if "custom_id" in interaction.data:
                if interaction.data["custom_id"] == "register":
                    ...
                    cha = await interaction.guild.create_text_channel(name="註冊代碼 "+interaction.user.name,category=category,overwrites=permissions)
                    ...
                    embed.add_field(name="請輸入 **/register** 填寫資料開始註冊",value="需求資料\n```學校:\n姓名:\n學號:\n年級:\n是否願意接受通知:```", inline=False)
    ...
    @app_commands.describe(area="選擇學校地區",school="輸入學校",name="請輸入名字(真名匿名皆可)",student_id="請輸入學號(末三碼請以***替換)",grade="選擇年級",tag="是否接受通知")
    ...
    @app_commands.describe(mid="註冊ID",school="修改學校",name="修改名字",student_id="修改學號",grade="修改年級",tag="修改通知")
    async def edit_register(self, interaction:discord.Integration, mid:str,area:Optional[str], school:Optional[str], name:Optional[str], student_id:Optional[str], grade:Optional[str], tag:Optional[str]):
        try:
            # 取得要編輯的訊息
